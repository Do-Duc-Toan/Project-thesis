...
uint16_t initial_speed = 0;
uint8_t real_speed = 0;
uint8_t stop_obstacle = 0, obstacle = 0; //state, obstacle, position
uint8_t location = 0;
uint8_t stop_command = 0;
uint8_t AGV_navigation = 0;
int time_run = 0;
char transmitting_data[50];
char RxData[50];
uint8_t temp[2];
int indx = 0;
...
void turn_left() {
	if (sensor_value == 0b11011) { //error = 0
		TIM1->CCR1 = initial_speed;
		TIM1->CCR2 = initial_speed;
		TIM1->CCR3 = 0;
		TIM1->CCR4 = 0;
	...
	else if (sensor_value == 0b10111 || sensor_value == 0b10011) { //error = -1 or error = -2
		TIM1->CCR1 = 0;
		TIM1->CCR2 = initial_speed + 100;
		TIM1->CCR3 = initial_speed - 100;
		TIM1->CCR4 = 0;
		obstacle_stop();
	}
	else if (sensor_value == 0b11101 || sensor_value == 0b11001) { //error = 1 or error = 2
		TIM1->CCR1 = initial_speed + 50;
		TIM1->CCR2 = 0;
		TIM1->CCR3 = 0;
		TIM1->CCR4 = 0;
	}
	else if (sensor_value == 0b00111 || sensor_value == 0b00011) { //error = -3
		TIM1->CCR1 = 0;
		TIM1->CCR2 = initial_speed + 200;
		TIM1->CCR3 = initial_speed;
		TIM1->CCR4 = 0;
	}
	else if (sensor_value == 0b01111) { //error = -4
		TIM1->CCR1 = 0;
		TIM1->CCR2 = initial_speed + 350;
		TIM1->CCR3 = initial_speed + 150;
		TIM1->CCR4 = 0;
	}
	else if (sensor_value == 0b11111) {
		TIM1->CCR1 = 0;
...
void turn_right() {
	...
	else if (sensor_value == 0b11110) { //error = -4
		TIM1->CCR1 = initial_speed + 350;
		TIM1->CCR2 = 0;
		TIM1->CCR3 = 0;
		TIM1->CCR4 = initial_speed + 150;
